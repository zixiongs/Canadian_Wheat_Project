...
#levels(as.factor(biomass_2022$genotype))[1]

current_genotype <- biomass_2022 |> filter(genotype == "AAC_Goodwin")
anova_result <- aov(above_ground_biomass ~ temperature_level*co2_level, data = current_genotype)
#summary(aov1)
...
biomass_2022 |> 
  ...
  annotate("text", x= 0.6, y=38, label="(a)",size = 5) +
  annotate("text", x= 1.2, y=35, label=sig_labs,size = 4.8,hjust=0) +
...
create_biomass_plot <- function(data, genotype_name, plot_label = "(a)") {
  data |> 
    ...
    annotate("text", x = 0.6, y = 38, label = plot_label, size = 5) +
    theme(
...
create_biomass_plot(biomass_2022, "AAC_Goodwin", "(a)")
...
p1 <- plot_grid(plotlist = plot_list, nrow = 5, ncol = 4)

p1 <- wrap_plots(plot_list, nrow = 5, ncol = 4)
